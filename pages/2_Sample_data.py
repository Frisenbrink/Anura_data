...
url = "pages/6_Vibreshark_download.py"
...
st.header("Anura™ sample data download")

st.markdown("""

We are pleased to provide you with access to data collected using the ReVibe Anura™ measurement platform. The downloadable files contain detailed measurements recorded by Anura sensors.

### Available Data Types

1. **Vibreshark .vibe Files**  
   - Collected using **Vibreshark** together with **VS10-C** sensors and the **TR11-USB** transceiver. 
   - Can be analyzed directly in **Vibreshark** https://revibe-anura.streamlit.app/Vibreshark_download
            
""")

def file_selector(folder_path='./vibe_files'):
...
with open(filename, "rb") as fp:
    btn = st.columns(3)[1].download_button(label=(
        "Download selected file " + filename[13:]),
        data=fp,
...
st.markdown("""           
2. **Raw Anura Data**  
   - Collected as raw data from the **Anura monitoring system** using **VS1** harvesting sensors and **TR1** transceivers.  
   - Can be analyzed using **Vibinspect** https://revibeenergy.com/vibinspect-analysing-software/ or other vibration analysis software.
